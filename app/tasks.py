...
def taskfile(taskid, name=''):
	...
	if not 'A5A4_TASKS' in app.config:
		folder = os.path.join(app.root_path, 'tasks')
	else:
...
def delpdf(taskid, idx):
	...
	task = get(taskid)
	if not task or not len(idx) == 1 or not idx in task.files:
		return False
	pdf = task.files[idx]
	# delete pdf
	try:
...
def generate(taskid):
	...
	if result != 0 or not os.path.isfile(tmpName) or os.stat(tmpName).st_size == 0:
		...
		command = [app.config['PDFJAM'], tmpName, '--nup', '2x1', '--landscape', '--a4paper', '--outfile', taskfile(taskid, RESULT)]
		process = subprocess.Popen(command, stderr=subprocess.PIPE)
		_, err = process.communicate()
		result = process.returncode
		if result != 0:
